...
@RestController
@CrossOrigin()
public class CommonUsersRightsController {
   
	
	public static final String CONTROLLER_URL = GenericUtility.APPLICATION_CONTEXT + "/admin/user/features";
	...
	private GenericUtility Utility; 
	 ...
	 @GetMapping(CONTROLLER_URL+"/users-view")
		public ResponseEntity<?> UsersView(@RequestHeader("authorization") String token)  { 
				...
				try {
					...
					response.DATA=UsersService.getAllInternalUsersDropDown();
	 ...
	 @GetMapping(CONTROLLER_URL+"/view.")
		public ResponseEntity<?> View(@RequestHeader("authorization") String token)  { 
				...
				try {
					...
					response.USER_MESSAGE="";
					response.SYSTEM_MESSAGE="";
					response.DATA=UsersFeaturesService.getAllFeaturesGroupWise();
	...
	public ResponseEntity<?> Save(Long[] userIds,Long[] userRightsIds,@RequestHeader("authorization") String token)  { 
				...
				try {
					...
					response.USER_MESSAGE="";
					response.SYSTEM_MESSAGE="";
					UsersFeaturesService.Save(Utility.parseLong(userIds),Utility.parseLong(userRightsIds),UserID);
	...
	@PostMapping(CONTROLLER_URL+"/single-user-view")
	public ResponseEntity<?> View(Long userId,Long roleId,@RequestHeader("authorization") String token)  { 
			        long UserID =Utility.getUserIDByToken(token);
					ResponseEntityOutput response=new ResponseEntityOutput();
					Map map=new HashMap<>();
					...
					try {
						...
						response.USER_MESSAGE="";
						response.SYSTEM_MESSAGE="";
						response.DATA= UsersFeaturesService.getAllUserAssignedFeaturesGroupWiseWithAllFeaturesForEditAndRoleId(Utility.parseLong(userId),Utility.parseLong(roleId));
	...
	public ResponseEntity<?> Delete(Long userId,@RequestHeader("authorization") String token)  { 
	        long UserID =Utility.getUserIDByToken(token);
			ResponseEntityOutput response=new ResponseEntityOutput();
			Map map=new HashMap<>();
			...
			try {
				...
				response.SYSTEM_MESSAGE="";
				UsersFeaturesService.deleteAllUserFeaturesRights(Utility.parseLong(userId));
